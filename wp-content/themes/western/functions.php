...
function get_custom_feed() {
	...
	// Events RSS Feed URL
    	$feed = fetch_feed('http://westernadvance.ca/calendar/western.xml');
    	if( ! is_wp_error( $feed ) ) {
...
// Add Featured Image to RSS feeds for Cascade Server integration
add_filter('rss2_item', 'add_featured_image_data_to_feed');
add_filter('atom_entry', 'add_featured_image_data_to_feed');
add_filter('rdf_item', 'add_featured_image_data_to_feed');
function add_featured_image_data_to_feed () {
    global $post;
    $thumbAlt = get_post_meta(get_post_thumbnail_id($post->ID), '_wp_attachment_image_alt', true);
    if($thumbAlt != null) {
        echo("<featuredImageAlt>{$thumbAlt}</featuredImageAlt>");
    } else {
        $blankthumb = 'Western News Featured Image for ' . $post->post_title;
        echo("<featuredImageAlt>{$blankthumb}</featuredImageAlt>");
    }
}
