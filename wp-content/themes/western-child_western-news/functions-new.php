    ...
    function widget($args, $instance) {
        ...
        ?>
              <?php echo $before_widget; ?>
					...
					$results = $wpdb->get_results("SELECT ID, post_date, post_title FROM {$wpdb->prefix}posts WHERE post_type = '". $post_type_for_query ."' AND post_status = 'publish' ORDER BY post_date DESC");
					...
					foreach($archiveArr as $year=>$months) {
					    ...
					    <h3><a href="<?php echo get_year_link($year) . '?post_type=' . $post_type_for_query; ?>"><?php echo $year; ?></a></h3>					    					    
					        <div class="posts_accordion-content"><ul class="archive-sub-menu">
					            <?php foreach( array_reverse($months) as $month=>$posts) { ?>
...
function hack_wp_title_for_home( $title )
{
  if( empty( $title ) && ( is_home() || is_front_page() ) ) {
    return __( 'Media Relations', 'theme_domain' );
  }
  return $title;
}
